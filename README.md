...
- опційно: `Завантажити міста` → вибір аркушів-міст або «Вибрати всі» (дані з кількох аркушів **об’єднуються** по назві рядка: суми додаються);
- кнопка `Синхронізувати`.

Якщо міста не завантажувати — як раніше береться **перший аркуш** джерела.

Якщо поле 2 порожнє, сервіс створює копію таблиці 1 і перетворює вибрані аркуші
у artist report: залишає компактну структуру A:E, видаляє зайві рядки/колонки
та переносить посилання з назв витрат.

API: у body можна передати `selected_cities: ["Berlin","Paris"]` (разом із `source_url` / `target_url`). Якщо поля немає — режим один аркуш.
